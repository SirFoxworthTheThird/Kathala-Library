...
import fellowshipLibrary from '../library/the-fellowship-of-the-ring.pwk?raw'
...
// One copy now. This checked the example and the published world separately,
// back when they were two files that could disagree; the split merged them.
const FILES: [name: string, raw: string][] = [
  ['library/the-fellowship-of-the-ring.pwk', fellowshipLibrary],
]
...
/*
  Removed with the split: this checked that the example copy and the library
  copy agreed about every scale, after they had diverged — the library's Middle
  Earth read 0.85 where the example's read 1.9449, so a reader who downloaded
  the book and a writer who imported the file were measuring the same map
  differently. There is one copy now, so there is nothing left to disagree.
*/
