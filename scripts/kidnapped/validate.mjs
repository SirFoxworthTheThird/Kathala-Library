...
import { narrativeText, normalize, countWords, sourceFileSha256 } from './source-text.mjs'

const root=path.resolve(import.meta.dirname,'../..'),file=path.join(root,'library/kidnapped.pwk'),data=JSON.parse(fs.readFileSync(file,'utf8')),fail=m=>{throw Error(m)},unique=a=>new Set(a).size===a.length
if(data.chapters.length!==30)fail('Expected 30 chapters')
if(data.events.length!==90||data.events.length!==data.sceneTexts.length)fail('Expected 90 events and scenes')
...
if(sceneWords!==sourceWords||sourceWords!==80696)fail(`Manuscript word count drift: ${sceneWords}/${sourceWords}`)
...
for(const c of data.chapters)if(data.events.filter(e=>e.chapterId===c.id).length!==3)fail(`Chapter does not have three events: ${c.title}`)
...
if(data.events.some((e,i,a)=>i&&e.inWorldTime<a[i-1].inWorldTime))fail('Event chronology is not monotonic')
...
for(const key of ['ransome','shuan','colin']){const ss=data.characterSnapshots.filter(s=>s.characterId===`kid-char-${key}`);if(!ss.some(s=>s.isAlive)||!ss.some(s=>!s.isAlive))fail(`${key} alive-state change missing`)}
const layers=new Map(data.mapLayers.map(m=>[m.id,m])),markers=new Map(data.locationMarkers.map(m=>[m.id,m]))
if(data.mapLayers.filter(m=>!m.parentMapId).length!==1)fail('Expected one Scotland root map')
...
for(const r of data.mapRoutes){if(!layers.has(r.mapLayerId)||r.waypoints.length<2)fail(`Invalid route ${r.name}`);for(const p of r.waypoints){if(!markers.has(p)||markers.get(p).mapLayerId!==r.mapLayerId)fail(`Route waypoint off-layer: ${r.name}`)}}
...
if(data.characters.some(c=>c.aliases.includes('Mr Thomson')||c.aliases.includes('The Red Fox')))fail('Future aliases leaked from standing character data')
