...
const root=path.resolve(import.meta.dirname,'../..'),file=path.join(root,'library/the-thirty-nine-steps.pwk'),data=JSON.parse(fs.readFileSync(file,'utf8')),fail=m=>{throw Error(m)},unique=a=>new Set(a).size===a.length
if(data.chapters.length!==10)fail('Expected ten chapters')
if(data.events.length!==72||data.sceneTexts.length!==72)fail('Expected 72 covered events/scenes')
...
if(sceneWords!==sourceWords||sourceWords!==40896)fail(`Manuscript word count drift: ${sceneWords}/${sourceWords}`)
...
const scudder=data.characterSnapshots.filter(s=>s.characterId==='steps-char-scudder');if(!scudder.some(s=>s.isAlive)||!scudder.some(s=>!s.isAlive))fail('Scudder alive-state change missing')
const layers=new Map(data.mapLayers.map(m=>[m.id,m])),markers=new Map(data.locationMarkers.map(m=>[m.id,m]))
if(data.mapLayers.filter(m=>!m.parentMapId).length!==1)fail('Expected one root map')
...
const ids=new Set;for(const [key,value] of Object.entries(data)){if(Array.isArray(value))for(const x of value){if(x?.id){if(ids.has(x.id))fail(`Duplicate record id ${x.id}`);ids.add(x.id)}}}
